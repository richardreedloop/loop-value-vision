...
export default function PerformanceTab({ data, onChange, onNext }: PerformanceTabProps) {
  ...
  const totalAnnualRevenue = localData.numberOfLocations * localData.averageRevenue
  const annualImprovement = totalAnnualRevenue * (localData.improvementPercentage / 100)
  const perLocationImprovement = localData.averageRevenue * (localData.improvementPercentage / 100)
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              min={1}
              step={1}
              value={localData.numberOfLocations}
              onChange={(e) => handleChange("numberOfLocations", Number(e.target.value))}
            />
            <p className="text-sm text-slate-500">The total number of business locations in your network.</p>
          </div>

          <div className="space-y-2">
            <Label htmlFor="averageRevenue">Average Annual Revenue Per Location (£)</Label>
            <Input
              id="averageRevenue"
              type="text"
              inputMode="numeric"
              value={localData.averageRevenue.toLocaleString()}
              onChange={(e) => {
                const value = Number(e.target.value.replace(/,/g, ''))
                if (!isNaN(value)) {
                  handleChange("averageRevenue", value)
                }
              }}
            />
            <p className="text-sm text-slate-500">The average annual revenue generated by each location in your network.</p>
          </div>
          ...
          <Card>
            <CardContent className="pt-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-slate-600">Annual Improvement Per Location:</span>
                  <span className="font-medium">£{perLocationImprovement.toLocaleString()}</span>
                </div>
              </div>
            </CardContent>
          </Card>
